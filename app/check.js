...
const { syntaxError, gameError, linePos } = require('./error.js')
...
const checkStart = (game) => {
	...
	if (game.start === undefined) {
		errors.push(gameError("game has no starting location"))
		return errors
	}
	if (game.entities[game.start]?.type !== 'location')
		errors.push(gameError("game start is not a location"))
	return errors
}
...
const checkLocationExits = (location) => {
	...
	Object.keys(exits).forEach(dir => {
		...
		if (!(dest in location.game.entities))
			errors.push(gameError(`exit '${dest}' does not exist, in location '${location.name}'`, location))
	})
	return errors
}
...
const checkLocation = (location) => {
	...
	if (location.texts.length === 0)
		errors.push(gameError(`location '${location.name}' has no description`, location))
	if (isObjectEmpty(location.exits))
		errors.push(gameError(`location '${location.name}' has no exits`, location))
	else
...
const checkObject = (object) => {
	...
	if (object.texts.length === 0)
		errors.push(gameError(`object '${object.name}' has no description`, object))
...
const checkGameForErrors = (game) => {
	...
	if (game.text === undefined)
		errors.push(gameError("game has no introduction text"))
...
const checkNodeForErrors = (node) => {
	...
	node.children?.forEach((n) => errorsFound.push(checkNodeForErrors(n)))
